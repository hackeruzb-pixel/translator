...
translateBtn.addEventListener("click", () => {
  ...
  if (translateFrom === "auto") {
    doTranslate(text, "en", translateTo);
  } else {
    doTranslate(text, translateFrom, translateTo);
  }
});
...
function doTranslate(text, from, to) {
  const sourceLang = from === "auto" ? "en" : from.split("-")[0];
  const targetLang = to.split("-")[0];

  const apiUrl = `https://lingva.ml/api/v1/${sourceLang}/${targetLang}/${encodeURIComponent(
    text
  )}`;
  ...
  fetch(apiUrl)
    ...
    .then((data) => {
      if (data.translation) {
        ...
        toText.value = "Tarjima topilmadi.";
      }
    })
    .catch(() => {
      toText.value = "Xatolik yuz berdi.";
    });
}
...
icons.forEach((icon) => {
  icon.addEventListener("click", ({ target }) => {
    ...
    if (target.classList.contains("fa-copy")) {
      ...
      if (!value.trim()) return;
      const utterance = new SpeechSynthesisUtterance(value);
      utterance.lang = lang === "auto" ? "en" : lang.split("-")[0];
      speechSynthesis.speak(utterance);
    }
  });
});

function saveToHistory(text, translated) {
  console.log("Tarix:", text, "→", translated);
}
function doTranslate(text, from, to) {
  const sourceLang = from === "auto" ? "en" : from.split("-")[0];
  const targetLang = to.split("-")[0];
